...
static int kernel_monitor_shell(void);
static int kernel_monitor_parse(uint8_t*, uint8_t);
...
int pintos_init(void) {
    ...
    if (*argv != NULL) {
        ...
        printf("No command line passed to kernel.\nStart kernel monitor.\n");
        kernel_monitor_shell();
    }
...
/** When kernel monitor successfully get a command line, this function
    will parse and execute it. 
    If cmdline is "whoami", my stu ID will be printed.
    If cmdline is "exit", kernel monitor will quit.
    If cmdline is empty, shell will simply start another line.
    If any other, print "Invalid command."*/
static int kernel_monitor_parse(uint8_t* cmdline, uint8_t len) {
    if(strcmp((const char *)cmdline, "whoami") == 0) {
        puts("\nstu ID : 2000017737");
    }
    else if(strcmp((const char *)cmdline, "exit") == 0) {
...
/** When no command line passed to kernel, this shell will be started,
    kernel will run interactively, receiving keyboard inputs. */
static int kernel_monitor_shell(void) {
    console_init();
    input_init();
    uint8_t _buffer[64], bt = 0; // bt : buffer_top
    for (;;) {
        putbuf("PKUOS> ", 7);
        bt = 0;
        for (;;) { // reading 
        ...
        if (bt <= 63) {
            _buffer[bt] = 0;
            int res = kernel_monitor_parse(_buffer, bt);
            if (res > 0) {
                console_panic();
                return 0;
            }
